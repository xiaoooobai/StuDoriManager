...
<script>
    ...
    function deleteById(id) {
        ...
        $.post(
            '<%=request.getContextPath()%>/student?method=deleteById',
            {'id':id},
            function (result){
                if(result.code==0){
    ...
    layui.use('table', function(){
        ...
        table.render({
            elem: '#test'
            ,skin:'row'
            ,url:'<%=request.getContextPath()%>/student?method=selectByPage'
            ...
            ,cols: [[
                ...
                ,{field:'class_', title:'班级', width:110 }
        ...
        table.on('toolbar(test)',function (obj){
            ...
            if(obj.event==='add'){
                layer.open({
                    ...
                    type: 2,
                    area: ['550px', '500px'],
                    content: '<%=request.getContextPath()%>/Student/student_add.jsp'
                });
                ...
                layer.confirm('真的删除行么', function(index){
                    $.post(
                        '<%=request.getContextPath()%>/student?method=deleteAll',
                        {'ids': ids},
                        function(result) {
                            if (result.code == 0) {
        ...
        table.on('tool(test)', function(obj){
            ...
            if(obj.event === 'del'){
                ...
                layer.open({
                    ...
                    type: 2,
                    area: ['550px', '500px'],
                    content: '<%=request.getContextPath()%>/student?method=getStudentUpdatePage&id=' + data.id
                });
            }
        });
        var $ = layui.$, active = {
